    ...
    <div className="bg-white">
      ...
      <div className="max-w-lg mx-auto mt-10">
        <div className="flex justify-center items-center space-x-4">
          {/* Przycisk "Poprzednia" */}
          <Link
            to={`${location.pathname}?page=${response.current_page - 1}`}
            ...
            onClick={(e) => handleClick(e, response.current_page === 1)} // Blokuje kliknięcie, jeśli strona jest pierwsza
          >
            Poprzednia
          </Link>
          ...
          {pages(response.current_page, response.last_page).map((page) => (
            <Link to={`${location.pathname}?page=${page}`} key={page}>
              <span className={`cursor-pointer hover:text-blue-600 ${page === response.current_page ? "text-blue-600" : "text-gray-600"}`}> {page} </span>
            </Link>
          ))}
          ...
          <Link
            ...
            onClick={(e) => handleClick(e, response.current_page === response.last_page)} // Blokuje kliknięcie, jeśli strona jest ostatnia
          >
            Następna
          </Link>
        </div>
