    ...
    <>
      <nav className="bg-neutral-950 text-white p-4" onMouseLeave={() => {setDropDown(false)}}>
        ...
        <div className="flex items-center justify-between">
          ...
          {/* Searchbar */}
          <div class="relative ml-auto w-1/2 md:w-1/3 lg:w-1/4">
            <div class="absolute inset-y-0 left-0 flex items-center pl-3">
              <svg class="h-4 w-4 text-gray-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20">
                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m19 19-4-4m0-7A7 7 0 1 1 1 8a7 7 0 0 1 14 0Z" />
              </svg>
            </div>
            <input type="text" id="search-navbar" class="w-full rounded-lg border p-2 pl-10 text-sm text-gray-900 focus:border-blue-500" placeholder="Search..." />
          </div>
          {/* Other */}
                  ...
                  <div className="absolute top-full left-0 mt-2 bg-neutral-950 w-48 text-left z-10" onMouseLeave={() => {setDropDown(false)}}>
                    <ul>
                      <li className="hover:bg-neutral-800">
                        <Link to="products/category/1" className="block px-4 py-2 text-white hover:text-green-300">Sprot</Link>
                      </li>
                      <li className="hover:bg-neutral-800">
                        <Link to="products/category/2" className="block px-4 py-2 text-white hover:text-green-300">Moda</Link>
                      </li>
                      <li className="hover:bg-neutral-800">
                        <Link to="products/category/3" className="block px-4 py-2 text-white hover:text-green-300">Buty i uroda</Link>
                      </li>
                    </ul>
                  </div>
                )}
